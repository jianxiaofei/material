						...
						if("200"==data.code){
							...
							for(var i=0;i<length;i++){
								...
								if(1==d.type){
									...
									name = "无执法视频";
									color = "#e2e921";
								}else if("zuijia"==d.type){
									name = "醉驾";
									color = "#75f4ea";
								}
								var li = document.createElement('li');
								li.className = 'mui-table-view-cell mui-media mui-col-xs-3';
								li.innerHTML = '<a href="javascript:void(0);" data-key="'+d.type+'" class="submain"><div class="error-circle" style="background-color: '+color+';">'+
											   '<div class="title">'+name+'</div><div class="number">共'+count+'条</div></div></a>';
								
								leader_content.appendChild(li);
							}
							mui("#leader-content").on("tap","a.submain",function(){
								var type = this.getAttribute("data-key");
								if("zuijia"==type){
									document.location.href = "submain.php?type="+type;	
								}else{
									document.location.href = "list.php?type="+type;	
								}								
							});
						}else{
