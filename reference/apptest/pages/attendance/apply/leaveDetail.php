	...
	<body>
		<div class="mui-content mui-scroll-wrapper mui-fullscreen">
			...
			<div id="footer" class="datatom-footer mui-hidden">
				...
				<a id="xiaojia" class="datatom-item mui-hidden" href="javascript:;">
					<label class="datatom-label">销假</label>
				</a>
...
//						showWebviewToast(JSON.stringify(data));
						if(data.code == "200") {
							...
							if("1" == item.status) {
								...
								var diff_count = endtime_date.diff(current);
								if(diff_count > 0) {
									document.getElementById("footer").classList.remove("mui-hidden");
									document.getElementById('xiaojia').classList.remove("mui-hidden");
								}
							}
							...
							var auditor = item.auditor;
							//			showWebviewToast("auditor>>" + auditor);
							if(null != auditor) {
								html += '<div class="mui-row mui-text-center">';
								html += '<div class="mui-col-sm-3 mui-col-xs-4">';
								html += '<label>审批人员：</label></div>';
								html += '<div class="mui-col-sm-9 mui-col-xs-8">';
								//			showWebviewToast("auditor>>---dw");
								//审批人列表
								var auditor_list = JSON.parse(item.auditorlist);
								var temp_boolean = true;
								//			showWebviewToast("auditor_list>>" + auditor_list);
								mui.each(auditor_list, function(index_temp, item_temp) {
									//					showWebviewToast("item_temp.policenum>>" + item_temp.policenum + "auditor>>" + auditor);
									if(item_temp.policenum == auditor) {
										html += '<p class="datatom-text-left">' + item_temp.realname + '</p></div></div>';
										temp_boolean = false;
										return false;
									}
								});
								if(temp_boolean) {
									html += '<p class="datatom-text-left">已完成</p></div></div>';
								}
							}
				...
				//销假
				document.getElementById("xiaojia").addEventListener("tap", function() {
					mui.ajax(basePath + '/osapi/leave.php', {
						data: {
							action: 'endleaveinadvance',
							id: id,
							reason: reason
						},
						beforeSend: function(request) {
						...
						timeout: 10000, //超时时间设置为10秒；
						success: function(data) {
							var result_str=data.result;
							if(data.code == "200") {
								window.location.reload();
								showWebviewToast("销假成功");
							} else {
								showWebviewToast(data.msg);
							}
							showWebviewToast(result_str);
