	...
	<body>
		...
		<script>
			...
			(function($) {
					...
					var applytime = <?php echo "'". $_GET["applytime"] . "'" ?>;
					var apply_content = <?php echo "'". $_GET["content"] . "'" ?>;
					console.info("content>>"+apply_content);
					...
					html += '<p class="datatom-text-left">' + date + '</p></div></div>';

					html += '<div class="mui-row mui-text-center">';
					html += '<div class="mui-col-sm-3 mui-col-xs-4">';
					html += '<label>申请时间：</label>';
					html += '</div><div class="mui-col-sm-9 mui-col-xs-8">';
					html += '<p class="datatom-text-left">' + applytime + '</p></div></div>';

					html += '<div class="mui-row mui-text-center">';
					html += '<div class="mui-col-sm-3 mui-col-xs-4">';
					html += '<label>异常内容：</label></div>';
					html += '<div class="mui-col-sm-9 mui-col-xs-8">';
					html += '<p class="datatom-text-left">' +apply_content+ '</p></div></div></div>';
					html += '<div class="datatom-line"></div>';
					
					content.innerHTML = html;
					//否决
					document.getElementById("nopass").addEventListener("tap", function() {
						document.location.href = "finishRecord.php?id=" + id+"&status=" + 4 ;
					});
					//同意
					document.getElementById("pass").addEventListener("tap", function() {
						document.location.href = "finishRecord.php?id=" + id+"&status=" + 3 ;
					});

	

			setTitle('工作日志异常审批详情'); //设置title
