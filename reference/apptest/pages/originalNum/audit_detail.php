...
* */
	...
	<body>
		<div >
			<div class="mui-scroll " id="content" style="margin-bottom: 50px;">
				<div class="mui-loading">
					<div class="mui-spinner mui-scroll" >
		...
		<div class="datatom-footer" id="footer" style="border-bottom: 1px solid #888;">
			<a id="confirm" class="datatom-item" href="javascript:void(0);">
				<label class="datatom-label">同意</label>
			</a>
			<a id="refuse" class="datatom-item" href="javascript:void(0);">
		...
		<script>
			...
			(function($) {
				...
				mui.ajax(basePath + '/osapi/firstCode.php', {
					...
					success: function(data) {
						if(data.code == "200") {
							...
							if(item.otherpictureid != null && item.otherpictureid != '') {
							...
							html += '<div class="mui-row mui-text-content"style="padding-left:10px;padding-top:10px;">'
							html += '<div class="mui-col-sm-3 mui-col-xs-4">'
							html += '<label> 车辆识别代号:</label>'
							html += '</div><div class="mui-col-sm-1 mui-col-xs-1">'
							html += '</div><div class="mui-col-sm-8 mui-col-xs-7"><label>'
							html += item.carshelfcode + '</label></div></div>'
							html += '<div class="mui-row mui-text-content"style="padding-left:10px;padding-top:10px">'
							html += '<div class="mui-col-sm-3 mui-col-xs-4">'
							html += '<label> 无法使用原号原因:</label>'
							html += '</div><div class="mui-col-sm-1 mui-col-xs-1">'
							html += '</div><div class="mui-col-sm-8 mui-col-xs-7"><label>'
				...
				setTitle('审批原号'); //设置title

			})(mui);
			//同意
			document.getElementById('confirm').addEventListener("tap", function() {
				window.location.href = "finishTask.php?id=" + id + "&status=2";
			});
			//否决
			document.getElementById('refuse').addEventListener("tap", function() {
				window.location.href = "finishTask.php?id=" + id + "&status=3";
			});
			//取消
